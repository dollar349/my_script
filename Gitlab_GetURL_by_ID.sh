...
{
    echo ""
    echo "This script helps to get the repo's URL by Gitlab project ID"
    echo "  Usage: $(basename $0) \${Gitlab project ID}"
}
...
curl -X GET -s -H "PRIVATE-TOKEN:${ACCESS_TOKEN}" "https://gitlab.com/api/v4/projects/${1}" | jq .web_url
